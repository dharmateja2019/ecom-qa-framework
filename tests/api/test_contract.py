...
def test_products_list_matches_contract():
    ...
    assert result["valid"], \
        f"Products list contract violation: {result['error']}"
...
def test_single_product_matches_contract():
    ...
    assert result["valid"], \
        f"Product contract violation: {result['error']}"
...
def test_auth_response_matches_contract():
    ...
    assert result["valid"], \
        f"Auth response contract violation: {result['error']}"
...
def test_contract_catches_missing_field():
    ...
    assert result["valid"] == False, \
        "Schema validator should catch missing price field"
